...
| CORR-01 | non-x86-asm `scalar_mul` KAT divergence masked by `continue-on-error` | **REAL (project-acknowledged) — fix pending** | `security-audit.yml` L143 `continue-on-error: true` on `test_large_scalar` + L78 TODO confirm a known sanitizer-only divergence in the portable (non-`__int128`) scalar path. Fix = reproduce under `-DSECP256K1_USE_ASM=OFF`+ASan/UBSan, locate the limb-carry/Barrett defect, fix without regressing the asm path. Deep, careful — scheduled as its own effort. |
