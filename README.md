...
- Every commit runs 1,000,000+ assertions across 132 non-exploit audit modules and 273 exploit PoCs ( 407 modules total; count via `python3 ci/sync_module_count.py`; canonical data: `docs/canonical_numbers.json`)
...
- All 134 non-exploit audit modules and all 273 exploit PoCs return `AUDIT-READY (self-generated)` status as of the last CAAS gate run. Zero failures — see pinned evidence: [`docs/EXTERNAL_AUDIT_BUNDLE.json`](docs/EXTERNAL_AUDIT_BUNDLE.json).

### Exploit PoC Test Suite (273 Tests, 20+ Coverage Areas)

In addition to the 407-module `unified_audit_runner`, UltrafastSecp256k1 ships **273 exploit-style PoC modules files** that actively try to break the library across its highest-risk surfaces. Each `audit/test_exploit_*.cpp` target builds and runs standalone so failures stay easy to attribute and reproduce.
